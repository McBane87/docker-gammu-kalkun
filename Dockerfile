...
RUN apt-get update \
    && apt-get install -y tzdata systemd cron logrotate rsyslog \
    && apt-get clean \
...
RUN BASE=/etc/systemd/system && \
	for i in {apache2,cron,gammu-smsd,mariadb,rsyslog}; do \
		mkdir $BASE/${i}.service.d && echo -e '[Unit]\nStartLimitBurst=3\nStartLimitIntervalSec=10\n\n[Service]\nRestart=always\nRestartSec=5' > $BASE/${i}.service.d/override.conf \
	; done
